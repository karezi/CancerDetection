...
class TifSearch(object):
    ...
    def show_region(self, x, y, w, h, level = 0):
        slide = openslide.OpenSlide(self.tif_img_files)
        mask = openslide.OpenSlide(self.tif_mask_files)
        level_count_slide = slide.level_count
        level_count_mask = mask.level_count
        ...
        print (m_img, n_img)
        print (m_mask, n_mask)
        tile_img = numpy.array(slide.read_region((x, y), level, (w, h)))
        tile_mask = numpy.array(mask.read_region((x, y), level, (w, h)))
        plt.figure(1)
        ax1 = plt.subplot(121)
        ax2 = plt.subplot(122)
        plt.sca(ax1)
...
if __name__ == '__main__':
    ts = TifSearch()
    # ts.show_whole(6)
    ts.show_region(32000, 64000, 399, 399, 0)
